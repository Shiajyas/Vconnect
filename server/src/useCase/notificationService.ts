...
export class NotificationService implements INotificationService {
  ...
  constructor(io: Server, userRepository: ISUserRepository, mainRepo: IUserRepository) {
    this.io = io;
  ...
  async sendNotification(
    ...
    try {
      ...
      const validReceivers = allReceivers.filter((receiver) => receiver !== null);
  
      console.log("✅ Valid Receivers:", validReceivers);
  
      if (!validReceivers.length) {
        console.log(`⚠️ No valid receivers found. Notification will not be stored.`);
        return;
      }
  
      // Extract user IDs for storage
      const receiverUserIds = validReceivers.map((receiver) => receiver?._id).filter(Boolean);
  
      if (receiverUserIds.length === 0) {
        console.log(`⚠️ No valid receiver IDs found. Skipping notification storage.`);
        return;
      }
      ...
      validReceivers.forEach((receiver) => {
        if (receiver?.socketId) {
          ...
          console.log(`⚠️ User ${receiver?._id} is offline. Notification stored in DB.`);
        }
      });
    } catch (error) {
