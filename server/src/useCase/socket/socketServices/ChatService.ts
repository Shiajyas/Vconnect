...
export class ChatService implements IChatService {
  ...
  constructor(chatRepository: IChatRepository,userRepository: ISUserRepository, ioInstance: Server) {
  ...
  async sendMessage(
    ...
    try {
      ...
      if (!chat.isGroupChat) {
        const foundUser = chat.users.find((user: any) => user?._id.toString() !== senderId);
      ...
      if (onlineReceiver) {
        ...
        if (user?.socketId) {
          ...
          socket.to(user.socketId).emit("chatUpdated", newMessage);
        }
      }
    } catch (error) {
  ...
  async editMessage(socket: Socket, chatId: string, messageId: string, newContent: string) {
    try {
      const updatedMessage = await this.chatRepository.editMessage(messageId, newContent);
