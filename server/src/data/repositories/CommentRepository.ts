...
export class CommentRepository implements ICommentRepository {
  ...
  async deleteComment(commentId: string): Promise<boolean> {
    try {
      ...
      if (!comment.parentCommentId) {
        const replies = await CommentModel.deleteMany({ parentCommentId: commentId });
      ...
      if (deleted) {
        ...
        if (post) {
          const newCommendCount = Math.max(0, (post.commendCount || 0) - deletedCount); // Ensure it never goes below 0
  ...
  async getCommentsForPost(postId: string, page: number, limit: number): Promise<any[]> {
    try {
  ...
  async addReply(parentCommentId: mongoose.Types.ObjectId, replyId: string): Promise<void> {
    try {
      if (!mongoose.Types.ObjectId.isValid(parentCommentId) || !mongoose.Types.ObjectId.isValid(replyId)) {
  ...
  async likeComment(commentId: string, userId: string): Promise<{ commentId: string; likes: mongoose.Types.ObjectId[]  }> {
    try {
  ...
  async unLikeComment(commentId: string, userId: string): Promise<{ commentId: string; likes: mongoose.Types.ObjectId[]  }> {
    try {
      if (!mongoose.Types.ObjectId.isValid(commentId) || !mongoose.Types.ObjectId.isValid(userId)) {
        throw new Error("Invalid comment or user ID");
      }
  ...
  async getCommentDetails(commentId: string): Promise<any> {
   let res = await CommentModel.findById(commentId).select("postId  parentCommentId").lean();
   console.log(res,">>>>>>>>123")
   return res
  }
