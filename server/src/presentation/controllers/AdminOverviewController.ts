  ...
  export const getAdminOverview = async (req: Request, res: Response) => {
    try {
      ...
      const data = await service.getOverview(range, { min: minLikes, max: maxLikes });
  ...
  export const getMostLikedPosts = async (req: Request, res: Response) => {
    try {
      ...
      const posts = await service.getMostLikedPosts(range, { min: minLikes, max: maxLikes });
  ...
  const posts = await service.getReportedPosts(range as "7d" | "1m" | "1y", page, limit);
