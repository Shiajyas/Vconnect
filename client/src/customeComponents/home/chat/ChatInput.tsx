  ...
  return (
    <div className={`relative p-3 mb-2 flex items-center w-full rounded-lg ${darkMode ? "bg-gray-800" : "bg-gray-100"}`}>
      ...
      <div ref={emojiPickerRef} className="relative">
        <Button type="button" onClick={() => setShowEmojiPicker((prev) => !prev)} disabled={isSending}>
          <Smile />
        </Button>
        {showEmojiPicker && (
      ...
      <label className="ml-2 cursor-pointer">
        <Paperclip />
        <input type="file" className="hidden" multiple onChange={handleFileChange} disabled={isSending} />
      </label>

      {/* Preview files grid */}
      {files.length > 0 && (
        <>
          <div className="fixed inset-0 bg-black bg-opacity-30 z-10" onClick={() => !isSending && setFiles([])} />
          <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[90vw] max-w-lg bg-white dark:bg-gray-900 p-4 rounded-xl shadow-xl z-20 max-h-[80vh] overflow-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {files.map(({ file, status }, idx) => (
                <div key={idx} className="relative border rounded-md p-2 bg-gray-50 dark:bg-gray-800 flex flex-col items-center">
                  {!isSending && (
                    <button
